...
import org.lera.etl.util.{IBPException, KuduUtils, TableRunInfo}
...
object IBPMain {
  ...
  private val logger : Logger = Logger.getLogger(IBPMain.getClass)
  ...
  def getControl(sourceSystem : String) : (Int, Int) = {
    val tableName = s"$ibpAuditDatabase.${getProperty(spark.job_exec_control_table)}"
    val controlDf = KuduUtils
    .readKuduWithCondition(tableName, where = s"lower(source_system)- '${sourceSystem.toLowerCase()}'")

    if(controlDf.isEmpty){

      val defaultNotificationTime : Int =
        Try(getProperty("spark.ibp_default_notification_interval_time")).getOrElse("30").toInt

      val defaultIntTime : Int =
        Try(getProperty("spark.ibp_default_max_run_time")).getOrElse("120").toInt
      ...
      controlDf
      .collect()
      .map(row => (row.getAs[Int] (fieldName = "notification_interval"), row.getAs[Int] (fieldName)))
      .head
    }
  }
  ...
  def ingestionMonitor(ingestionThread : Future[Unit],
    ...
    while(!ingestionThread.isCompleted){
      ...
      if(diffMinutes >= timeTaken){
        ...
        val message = s"Running more than ${s"${getHours()} ${getMinutes()}".trim}"
        sendEmailNotification(config, sourceSystem, message, runTime())
      }

      if(diffMinutes >= maxRunTime) {
        val updatedConf = config.map(conf =>
        conf.copy(
    ...
    ContextCreator.close()
    if(failedJobs.isEmpty) {
      logger.info(s"Data loading is completed for the source : $sourceSystem ")
      sendEmailNotification(config,sourceSystem,message = "completed", runTime())
    } else {
      sendEmailNotification(
      config.map(conf => {
         val failedConf = failedJobs.filter(info => info.tableName == conf.target_table)
  ...
  private def sendEmailNotification(config : ParSeq[TableConfig],
    ...
    EmailSMTPClient
      .sendMail(createHTMLBody(config.seq,runTime,message),completeSubject,recipients)
  }
  ...
  def getTableConfig(args: Array[String]):(String, ParSeq[TableConfig]) = {
    ...
    Try{
      ...
      val tableNames : Array[String] =
        Try(args(3)).getOrElse(StringExpr.empty).split(StringExpr.comma).filterNot(_.isEmpty)
      ...
      case Failure(exception) =>
        ...
        val emptyConf : TableConfig = getEmptyTableConfig
          .copy(source_system = sourceSystem, sourcedata_regionname = region, message = error)

        sendEmailNotification(Seq(emptyConf).par, sourceSystem, message= "failed", runTime= "")
        throw exception
    }

  }

  def startIngestionProcess(sourceSystem : String, tableConfigs : ParSeq[TableConfig]): Unit = {

    logger.info("Database and table information are parsed from the config table")
    val rawData : ParSeq[(TableConfig, DataFrame)] = tableConfigs
      ...
      val transformedData : ParSeq[(TableConfig, DataFrame)]=  getTransformers(sourceSystem)
