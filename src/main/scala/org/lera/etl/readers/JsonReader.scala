...
object JsonReader extends Reader with PipelineBase{
  ...
  override def readData(properties : Properties, spark : SparkSession) : DataFrame = {
    ...
    val dataFrame : DataFrame = properties.getProperty("load_type_json").toLowerCase match{
      ...
      case "incremental" | "incr" =>
        ...
        val sourceDataFrame: DataFrame = readJsonFile(spark,filePath,multiLine)
  ...
  def readJsonFile(spark : SparkSession,filePath : String,multiLine : String) : DataFrame = {
   var df_json = spark.read
    ...
    while(nested_column_count !=0) {
      ...
      for(column_name <- df_json.schema.fieldNames) {
        ...
        else if(df_json.schema(column_name).dataType.isInstanceOf[StructType]) {
          nested_column_count_temp +=1
        }
      }
