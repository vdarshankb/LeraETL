...
object ExcelReadr extends Reader with PipelineBase{
  ...
  override def readData(properties : Properties, spark : SparkSession) : DataFrame = {
    ...
    val treatEmptyValueAsNulls = properties.getProperty("treatEmptyValuesAsNulls") // Optional, default: true
    ...
    val dataFrame : DataFrame = properties.getProperty("load_type_excel").toLowerCase match{
      ...
      case "incremental" | "incr" => 
        ...
        val sourceDataFrame: DataFrame = readExcelFile(spark,filePath,excelLibrary,dataAddress,header,treatEmptyValueAsNulls,inferSchema,maxRowsInMemory,excerptSize)
        if(incrementalCondition.nonEmpty)
          sourceDataFrame.where(incrementalCondition)
        else
          sourceDataFrame
      case "fullloadtype" => readExcelFile(spark,filePath,excelLibrary,dataAddress,header,treatEmptyValueAsNulls,inferSchema,maxRowsInMemory,excerptSize)
    }
  ...
  def readExcelFile(spark : SparkSession, filePath : String,excelLibrary : String,dataAddress : String,header : String,treatEmptyValueAsNulls : String,inferSchema : String,maxRowsInMemory : String,excerptSize : String) : DataFrame = {
  ...
  def readExcelBySheets(spark : SparkSession, properties : Properties, sheetNum : Int, mainDF : DataFrame) : DataFrame = {
    ...
    .option("treatEmptyValuesAsNulls", properties.getProperty("treatEmptyValuesAsNulls"))
...
//    Function call to read excel by sheets 
	  ...
	  {
	    dfPerSheet = dfPerSheet :+ readExcelBySheets(spark,properties,i,excelDF)
	  }
