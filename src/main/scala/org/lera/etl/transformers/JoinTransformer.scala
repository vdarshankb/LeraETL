...
object JoinTransformer extends JoinBaseTransformer{
  ...
  def getJoiningTableDetails(tableConf : TableConfig,
                             configDataFrame : DataFrame) : Seq[(JoinTable, Map[String,String])] = {
    ...
    .mapValues(_.map(lookup => (lookup.source_column, lookup.lookup_column)).toMap)
    .toSeq
