...
object LookupTransformer extends JoinBaseTransformer {
  ...
  override def transform(
    ...
    val lookupConfigDataFrame: DataFrame =
      configHandler(dataFrameSeq.keys)(
        getTableConfigFunc(lookupConfigTableName)
      )
  ...
  def lookupTransformations(
    tableConf: TableConfig,
    sourceDf: DataFrame,
    getJoinMap: Seq[(LookupGroup, Map[String, String])]
  ): DataFrame = {
    getJoinMap.foldLeft(sourceDf)(
      (df: DataFrame, lookupInfo: (LookupGroup, Map[String, String])) => {
        ...
        val joinedDf: DataFrame =
          sourceDf.join(lookUpDf, seqCon, joinType = "left")
